...
public class DrugQuery extends JInternalFrame {
	...
	private void setUpCompent() {
		...
		table = new JTable();
		String[] tableHeads = new String[] { "药品ID", "药品名称", "销售价", "进货价",
				"数量", "用途", "生产日期", "保质期", "生产厂家" };
	...
	private void updateTable(List list, final DefaultTableModel model) {
		...
		while (iterator.hasNext()) {
			...
			drugInfo = Dao.getDrugInfo(item);
			Vector rowData = new Vector();
			rowData.add(drugInfo.getDid().trim());
			rowData.add(drugInfo.getYpmc().trim());
			rowData.add(drugInfo.getXsj());
			rowData.add(drugInfo.getJhj());
			rowData.add(drugInfo.getSl());
			rowData.add(drugInfo.getYt().trim());
			rowData.add(drugInfo.getScrq().trim());
			rowData.add(drugInfo.getBzq().trim());
			ResultSet rs = Dao.query("select * from tb_gysInfo where gid='"
					+ drugInfo.getSccj().trim() + "'");
			String gname = null;
			try {
				if (rs != null && rs.next()) {
					gname = rs.getString("gysmc");
				}
	...
	private class QButtonActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (!qStr.isEmpty()) {
				List list = Dao.findForList("select * from tb_drugInfo where "
						+ tj + "='" + qStr + "'");
				updateTable(list, model);
			} else {
				allButton.doClick();
