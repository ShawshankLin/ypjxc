...
public class DrugRkQuery extends JInternalFrame {
	...
	private void setUpCompent() {
		...
		table = new JTable();
		String[] tableHeads = new String[] { "入库单号", "药品名称", "数量", "进货价",
				"进货金额" };
		...
		allButton.addActionListener(new ActionListener() {
			public void actionPerformed(final ActionEvent e) {
				textField.setText("");
				List list = Dao.getRuKuDetailInfos();
	...
	private void updateTable(List list, final DefaultTableModel model) {
		...
		RuKuDetailInfo rkDetailInfo;
		while (iterator.hasNext()) {
			List info = (List) iterator.next();
			Item item = new Item();
			item.setId((String) info.get(0));
			item.setName((String) info.get(1));
			rkDetailInfo = Dao.getRuKuDetailInfo(item);
			Vector rowData = new Vector();
			rowData.add(rkDetailInfo.getRid().trim());
			...
			rowData.add(ypmc);
			rowData.add(rkDetailInfo.getSl());
			rowData.add(rkDetailInfo.getJhj());
			rowData.add(rkDetailInfo.getJe());
	...
	private class QButtonActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (!qStr.isEmpty()) {
				List list = Dao.findForList("select * from tb_ruku_detaile where "
						+ tj + "='" + qStr + "'");
				updateTable(list, model);
			} else {
				allButton.doClick();
